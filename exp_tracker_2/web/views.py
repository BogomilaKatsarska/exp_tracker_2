...
from exp_tracker_2.web.forms import CreateProfileForm, EditProfileForm, DeleteProfileForm, CreateExpenseForm, \
...
def edit_profile(request):
    profile = get_profile()
    if request.method == 'POST':
        form = EditProfileForm(request.POST, request.FILES, instance=profile) #TODO: add in form: enctype="multipart/form-data"
        if form.is_valid():
            form.save()
            return redirect('show index')
    else:
